...
type UploadHandler struct {
	uploadDir   string          // 上传文件的存放路径
	maxSizeMB   int             // 文件最大大小
	allowedExts map[string]bool // 允许的文件格式
...
func NewUploadHandler(uploadDir string, maxSizeMB int, allowedExts []string, repos ...repository.MySQLRepo) *UploadHandler {
	...
	for _, ext := range allowedExts {
		extMap[strings.ToLower(ext)] = true // 文件格式都转为小写进行存放
	}
...
type uploadAvatarResponse struct { // 上传头像的响应格式
	URL      string `json:"url"`
...
func (h *UploadHandler) UploadAvatar(c *gin.Context) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "request body too large") { // 错误类型中出现了"request body too large"字样
			Error(c, http.StatusBadRequest, CodeInvalidParam, fmt.Sprintf("文件大小超过上限（%dMB）", h.maxSizeMB))
			return
		}
		Error(c, http.StatusBadRequest, CodeMissingParam, "请选择要上传的文件") // 请求中没有上传文件对象
		return
	}
	defer file.Close()

	// 校验扩展名
	ext := strings.ToLower(strings.TrimPrefix(filepath.Ext(header.Filename), ".")) // 获取文件的扩展名并转为小写
	...
	// 保存文件
	dst, err := os.Create(savePath) // 创建空文件
	if err != nil {
	...
	written, err := io.Copy(dst, file) // 把需要保存的文件写入到创建好的空文件里，返回的是写入的字节数
	if err != nil {
		os.Remove(savePath)
	...
	if h.userRepo != nil {
		...
		if err != nil || user == nil { // 没有获取到用户对象就会触发回滚：把已经落盘在服务器的文件删掉
		...
		if err := h.userRepo.UpdateUser(c.Request.Context(), user); err != nil {
			_ = os.Remove(savePath) // 没有成功写入数据库就会触发回滚：把已经落盘在服务器的文件删掉
			Error(c, http.StatusInternalServerError, CodeInternalError, "保存头像资料失败")
			return
		}
	}
